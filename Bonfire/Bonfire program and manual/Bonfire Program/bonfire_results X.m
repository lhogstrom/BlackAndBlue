...
set(figure(10),'Position',[160 170 window_size]);
...
% Plot the branch point path-length analysis
figure(40); clf; hold on;
window_size = [900 600];
B_path_length_plot_NperC(p_SWC_total,N_list,50);
set(figure(40),'Position',[180 80 window_size]);
subplot(2,1,1); legend(condition_list); legend boxoff;

figure(41); clf; hold on;
window_size = [750 500];
B_path_length_plot_cpdf(p_SWC_total,200);
set(figure(41),'Position',[200 60 window_size]);
subplot(2,1,1); legend(condition_list); legend boxoff;

figure(50); clf; hold on;
color_ind = gray_cl(length(each_cell_data));
for ii = 1:length(each_cell_data);
    scatter(each_cell_data{ii}(:,1),each_cell_data{ii}(:,3),100,color_ind(ii,:),'filled');
end
set(figure(50),'Position',[220 40 750 500]);
legend(condition_list,'Location','NorthEastOutside'); legend boxoff;
xlabel('Number of Processes'); ylabel('Total Cable Length');